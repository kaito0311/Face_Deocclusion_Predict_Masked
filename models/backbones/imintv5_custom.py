...
__all__ = ['iresnet18', 'iresnet34', 'iresnet50',
           'iresnet100', 'iresnet124', 'iresnet200']
...
class ConvBNBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        if activation == 'PReLU':
            ...
            raise NotImplementedError(
                "Not support activation {}".format(activation))
...
def make_upsample_layer(inplanes, planes, kernel_size=3, activation=None):
    assert activation is not None
    convbn = nn.ConvTranspose2d(
        inplanes, planes, kernel_size, stride=2, padding=1)
    if activation == 'PReLU':
        act = nn.PReLU(planes)
    elif activation == 'LeakyReLU':
...
def make_decoder_layer(inplanes, planes, n_blocks=1, activation='PReLU'):
    assert activation is not None
    if inplanes != planes:
        first_convbn = ConvBNBlock(
            inplanes=inplanes, planes=planes, activation=activation)
        layers = [first_convbn]
    else:
        layers = []
...
def iresnet160_custom(pretrained=False, progress=True, **kwargs):
